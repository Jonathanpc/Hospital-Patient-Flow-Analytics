...
# Read silver data (assume append_only)
...
silver_df = (
    silver_df
    .withColumn("row_num", F.row_number().over(w))  # Rank by latest admission time
    .filter(F.col("row_num") == 1)                  # Keep only lastest admission
    .drop("row_num")
)

# Patient Dimension Table Creation
# Prepare incoming dimension records (duplicated per patient, latest record)
...
LEFT JOIN incoming_patient_tmp i
...
if changed_keys:
    # Update exisiting current records: set is_current = false and effective_to = current_timestamp()
...
""").withColumn("surrogate_key", F.monotonically_increasing_id()) \
    .withColumn("effective_to", lit(None).cast("timestamp")) \
    .withColumn("is_current", lit(True)) \
    .select("surrogate_key", "patient_id", "gender", "age", "is_current", "effective_from", "effective_to")
...
# Prepare incoming(lastest per patient feed snapshot)
...
fact_enriched = fact_enriched.withColumn("length_of_stay_hours",
                             ...
                             .withColumn("is_currently_admitted", F.when(col(
                                 "discharge_time") > current_timestamp(), lit(True)).otherwise(lit(False))) \
